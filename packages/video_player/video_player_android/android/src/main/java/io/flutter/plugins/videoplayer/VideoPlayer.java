...
public abstract class VideoPlayer implements VideoPlayerInstanceApi {
  ...
  public static ExoPlayer constructExoPlayer(
    ...
    DefaultLoadControl.Builder loadControlBuilder = new DefaultLoadControl.Builder().setBufferDurationsMs(
    ...
    if (options.backBufferDurationMs != null && options.backBufferDurationMs > 0) {
      ...
      int backBufferInt = (int) Math.min(options.backBufferDurationMs.longValue(), Integer.MAX_VALUE);
      ...
      if (isHTTP(uri) && options.enableCache) {
        cacheDataSourceFactory =
                new CacheDataSourceFactory(
                        ...
                        options.maxFileBytes , options.cacheDirectory,  asset.assetUrl, loggerOptions, httpVideoAsset.httpHeaders);
