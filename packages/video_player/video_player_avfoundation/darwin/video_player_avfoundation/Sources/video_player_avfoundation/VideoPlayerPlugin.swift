...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  // Owns the SJMediaCacheServer-backed HTTP(S)/HLS proxy caching integration; see
  // FVPVideoPlayerCacheManager for why that dependency is kept out of this Swift file.
  private let cacheManager = VideoPlayerCacheManager()
  ...
  func setCacheOptions(_ msg: CacheOptionsMessage) throws {
    cacheManager.setEnableCache(
      msg.enableCache,
      cacheDirectory: msg.cacheDirectory,
      maxCacheBytes: msg.maxCacheBytes,
      maxFileBytes: msg.maxFileBytes
    )
  }

  func preloadIntoCache(
    uri: String,
    segmentCount: Int64,
    httpHeaders: [String: String],
    completion: @escaping (Result<Void, Error>) -> Void
  ) {
    cacheManager.preloadURL(uri, segmentCount: Int(segmentCount), httpHeaders: httpHeaders) {
      error in
      if let error = error {
  ...
  func cancelPreload(forURL uri: String) throws {
    cacheManager.cancelPreload(forURL: uri)
  }
  ...
  private func playerItem(with options: CreationOptions) throws -> FVPAVPlayerItem {
    ...
    // Swaps in the SJMediaCacheServer local proxy URL when cache is enabled for a cacheable
    // http(s) source; local file:// asset URLs are returned unchanged.
    let resolvedURL = cacheManager.resolvedURL(for: url)
    let asset = avFactory.urlAsset(with: resolvedURL, options: itemOptions)
